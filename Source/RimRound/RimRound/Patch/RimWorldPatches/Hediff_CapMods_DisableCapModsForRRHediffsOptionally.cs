...
namespace RimRound.Patch
{
    ...
    internal class Hediff_CapMods_DisableCapModsForRRHediffsOptionally
    {
        ...
        static void ReduceMovementPenaltyByPerkLevels(List<PawnCapacityModifier> newList, Hediff hediff) 
        {
            ...
            if (pcmIndex != -1)
            {
                newList[pcmIndex].offset = Mathf.Min(0, newList[pcmIndex].offset + (comp.perkLevels?.PerkToLevels?["RR_Comfortable_Corpulence_Title"] * 0.03f * Mathf.Abs(newList[pcmIndex].offset) ?? 0) + (comp.perkLevels.PerkToLevels?["RR_HeavyRevian_Title"] * 0.6f ?? 0));
